...
def __generate_shadow_config(args, authorities, relays, tgen_servers, perf_clients, tgen_clients):
    ...
    config["general"]["environment"] = "OPENSSL_ia32cap=~0x200000200000000;EVENT_NOSELECT=1;EVENT_NOPOLL=1;EVENT_NOKQUEUE=1;EVENT_NODEVPOLL=1;EVENT_NOEVPORT=1;EVENT_NOWIN32=1"

    if args.atlas_path is None:
        config["topology"]["path"] = "{}/{}".format(CONFIG_DIRNAME, TMODEL_TOPOLOGY_FILENAME)
    else:
        config["topology"]["path"] = str(args.atlas_path)
...
def __server(args, server):
    ...
    host["options"]["bandwidth_up"] = "{} kilobit".format(host_bw_kbit)
    host["options"]["bandwidth_down"] = "{} kilobit".format(host_bw_kbit)
...
def __tor_relay(args, relay, orig_fp, is_authority=False):
    ...
    host = {}
    host["options"] = {}
    host["options"]["ip_hint"] = relay['address']
    host["options"]["country_code_hint"] = str(relay['country_code'])
    host["options"]["bandwidth_down"] = "{} kilobit".format(kbits)
    host["options"]["bandwidth_up"] = "{} kilobit".format(kbits)
